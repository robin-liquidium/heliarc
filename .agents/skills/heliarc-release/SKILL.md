...
A request to release authorizes the release commit, push, immutable version tag, GitHub draft, Apple notarization, and final GitHub publication. Preserve unrelated work and never move a published tag.
...
4. Commit and push `main`. Wait for Checks on that exact commit. Create and push one annotated `vVERSION` tag only after CI succeeds.
...
6. After publication, download every public asset into a fresh temporary directory. Verify `SHA256SUMS`, staples, Gatekeeper, signatures, architectures, versions, release evidence, and both stable latest-download URLs.
